...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    provider_states.insert("OpenAI", if std::env::var("OPENAI_API_KEY").is_ok() { ProviderState::Enabled } else { ProviderState::Disabled });
    provider_states.insert("Gemini", if std::env::var("GEMINI_API_KEY").is_ok() { ProviderState::Enabled } else { ProviderState::Disabled });
    provider_states.insert("Claude", if std::env::var("ANTHROPIC_API_KEY").is_ok() { ProviderState::Enabled } else { ProviderState::Disabled });

    run_tui(provider_states)?;
    Ok(())
}
